...
RUN sed -i '/en_US.UTF-8/s/^# //' /etc/locale.gen && \
    ...
    useradd -m -s /bin/bash default && echo "default:123" | chpasswd 
...
RUN <<'EOF_RUN'
    ...
    cat <<'EOF' > /home/default/.config/autostart/fcitx5.desktop
[Desktop Entry]
Name=Fcitx5
...
EOF
    ...
    echo 'export XDG_RUNTIME_DIR=/run/user/$(id -u)' >> /home/default/.bashrc
    if [ "$BUILD_KDE" = "min" ] || [ "$BUILD_KDE" = "conc" ] ; then
    mkdir -p /home/default/.config 
    cat <<'EOF' > /home/default/.config/kwinrc
[Compositing]
Enabled=false
EOF
    fi
...
usermod -a -G aid_inet,aid_net_raw,input,video,tty,sudo,droidspaces-gpu default || true
